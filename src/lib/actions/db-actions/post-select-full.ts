...
export async function postSelectFullAll(): Promise<{ message: string, success: boolean, posts: PostFull[] | null }> {
...
export async function postSelectFullBySlug(data: { slug: string }): Promise<{ message: string, success: boolean, post: PostFull | null }> {
    const results = await db
        .select({
            post: posts,
            author: users,
