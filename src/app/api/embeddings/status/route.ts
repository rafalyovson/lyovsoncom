...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const totalPublished = allPosts.totalDocs + allBooks.totalDocs + allNotes.totalDocs
    const totalWithEmbeddings =
      postsWithEmbeddings.totalDocs + booksWithEmbeddings.totalDocs + notesWithEmbeddings.totalDocs
    ...
    const status = {
      ...
      statistics: {
        ...
        overallCoveragePercentage:
          totalPublished > 0 ? Math.round((totalWithEmbeddings / totalPublished) * 100) : 0,
          ...
          posts: {
            ...
            coveragePercentage:
              allPosts.totalDocs > 0
                ...
                : 0,
            needingEmbeddings: allPosts.totalDocs - postsWithEmbeddings.totalDocs,
          },
          books: {
            ...
            coveragePercentage:
              allBooks.totalDocs > 0
                ? Math.round((booksWithEmbeddings.totalDocs / allBooks.totalDocs) * 100)
                : 0,
            needingEmbeddings: allBooks.totalDocs - booksWithEmbeddings.totalDocs,
          },
          notes: {
            ...
            coveragePercentage:
              allNotes.totalDocs > 0
                ? Math.round((notesWithEmbeddings.totalDocs / allNotes.totalDocs) * 100)
                : 0,
            needingEmbeddings: allNotes.totalDocs - notesWithEmbeddings.totalDocs,
          },
        },
      },
          ...
          sampleEmbeddings.docs.length > 0
            ? Math.round(
                sampleEmbeddings.docs.reduce(
                  (sum: number, doc: any) => sum + (doc.embedding_dimensions || 0),
                  0,
    ...
    if (status.statistics.overallCoveragePercentage < 100) {
      status.recommendations.push({
        ...
        action: 'Edit and save existing content to generate embeddings automatically',
      })
    }
    ...
    Object.entries(status.statistics.collections).forEach(([collection, stats]: [string, any]) => {
