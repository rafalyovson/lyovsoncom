...
async function getAuthorUsernames(
  ...
  if (Array.isArray(authors)) {
    for (const a of authors) {
      if (a && typeof a === 'object' && 'username' in a && typeof (a as any).username === 'string') {
        const u = (a as any).username as string
        if (u?.trim()) usernames.add(u.trim())
      } else if (typeof a === 'number' || typeof a === 'string') {
        try {
          const user = await req.payload.findByID({ collection: 'users', id: a as any })
...
export const revalidatePost: CollectionAfterChangeHook<Post> = async ({
  ...
  if (doc._status === 'published') {
    ...
    try {
      const authorUsernames = await getAuthorUsernames(req, doc.authors as any, (doc as any).populatedAuthors)
      ...
      payload.logger.error('Failed to revalidate author pages for post change', e)
    }
  }
  ...
  if (previousDoc?._status === 'published' && doc._status !== 'published') {
    ...
    try {
      ...
      payload.logger.error('Failed to revalidate author pages for previous post state', e)
    }
  }
...
export const revalidateDelete: CollectionAfterDeleteHook<Post> = async ({ doc, req }) => {
  ...
  try {
    const authorUsernames = await getAuthorUsernames(req as any, doc?.authors as any, (doc as any)?.populatedAuthors)
    if (authorUsernames.length) {
      revalidateTag('users')
      for (const username of authorUsernames) {
        revalidateTag(`author-${username}`)
